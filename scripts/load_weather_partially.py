...
def load_weather_partially(
    ...
    data_abies = load_raw_data(file_name=abies_file, seed=seed, data_volume=data_volume)
    print(f'Объём данных для загрузки по пихтовым древостоям: {data_abies.shape[0]}.')
    # загрузим данные для прочих сообществ  
    data_not_abies = load_raw_data(file_name=othersp_file, seed=seed, data_volume=data_volume)
    print(f'Объём данных для загрузки по прочим древостоям: {data_not_abies.shape[0]}.')
